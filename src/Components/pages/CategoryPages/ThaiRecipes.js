...
const ThaiRecipesContainer = styled.div`
	width: 100%;
...
const ThaiRecipesInnerContainer = styled.div`
	display: flex;
	flex-direction: row;
	flex-wrap: wrap;
...
const ThaiRecipes = () => {
	...
	const GetMyRecipes = async () => {
		try {
			...
			const updatedRecipesListArray = data?.foods.filter(
				(list) => list?.category === "658061fda2ae14d295402243"
			);
	...
	return (
		<Layout title={"ThaiRecipes"}>
			<ThaiRecipesContainer>
				<H1>Thai Recipes</H1>
				<L>
					Thai food is a symphony of flavors, a dance of spices that creates a
					harmonious and unforgettable experience.
				</L>
				<ThaiRecipesInnerContainer>
					<>
						{auth?.user ? (
							<>
