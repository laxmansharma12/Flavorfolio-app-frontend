...
const MexicanRecipesContainer = styled.div`
	width: 100%;
...
const MexicanRecipesInnerContainer = styled.div`
	display: flex;
	flex-direction: row;
	flex-wrap: wrap;
...
const MexicanRecipes = () => {
	...
	const GetMexicanRecipes = async () => {
		try {
			const updatedRecipesListArray = recipes?.foods.filter(
				(list) => list?.category === "65806238a2ae14d29540224c"
			);
	...
	return (
		<Layout title={"MexicanRecipes"}>
			<MexicanRecipesContainer>
				<H1>Mexican Recipes</H1>
				<L>
					Mexican cuisine is a rich tapestry of flavors and traditions, often
					misunderstood but deeply celebrated.
				</L>
				<MexicanRecipesInnerContainer>
